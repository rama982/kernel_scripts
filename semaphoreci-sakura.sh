...
KERNEL_DIR=${HOME}/android_kernel_xiaomi_sakura-caf
KERN_IMG=$KERNEL_DIR/out/arch/arm64/boot/Image.gz
ZIP_DIR=$KERNEL_DIR/AnyKernel2
DTB=$KERNEL_DIR/out/arch/arm64/boot/dts/qcom/msm8953-qrd-sku3.dtb
...
function fin() {
     ...
     tg_channelcast "<b>Note for Sakura user: If stuck on boootlogo after installing kernel flash Magisk</b>"
...
tg_channelcast "<b>GENOM MIUI</b> kernel new build!" \
	...
	"For device <b>SAKURA</b> (Redmi 6 Pro)" \
...
sed -e 's/gcc/strip/')"
  ...
  cp $DTB $ZIP_DIR/treble/msm8953-qrd-sku3-sakura.dtb
