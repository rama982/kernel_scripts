...
#
# Server kernel-building execution script
...
sudo apt update
sudo apt install clang-8 lldb-8 lld-8 bc ccache gcc-aarch64-linux-gnu -y
...
function tg_sendstick() {
    curl -s -X POST "https://api.telegram.org/bot$BOT_API_KEY/sendSticker" \
         -d sticker="CAADBQADigAD_gvjC03l1mkFyLqBAg" \
         -d chat_id="-1001382306754" >> /dev/null
}

# Announce the completion
function tg_yay() {
    tg_sendinfo "$(echo "Seem like compilation completed...")"
 }
...
tg_sendstick
tg_sendinfo "New build started on $(hostname) using compiler CLANG at commit $(git log --pretty=format:'"%h : %s"' -1) under branch ${keks}"
